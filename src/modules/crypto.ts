...
export const rotateKey = (currentKey: string): string => {
    ...
    // In a real scenario, a key derivation function might be used.
    // For this simulation, simple regeneration is sufficient.
    return generateKey();
};

/**
 * Encrypts a JSON payload using AES-256-CBC and signs it with HMAC-SHA256.
 * @param data The JSON object to encrypt.
 * @param hexKey A 64-character hex string (32 bytes) for encryption.
 * @returns A base64 string containing IV, HMAC, and ciphertext.
 */
...
/**
 * Decrypts a payload encrypted with the `encrypt` function.
 * Verifies HMAC integrity before decryption.
 * @param payload The base64 encoded payload.
 *  @param hexKey A 64-character hex string (32 bytes) for decryption.
 * @returns The original JSON object, or null if integrity check fails or decryption fails.
 */
export const decrypt = (payload: string, hexKey: string): object | null => {
    try {
        ...
        if (!crypto.timingSafeEqual(receivedMac, expectedMac)) {
            logger.error('Module [Crypto]: Decryption failed: HMAC verification failed. Data may have been tampered with.');
            return null;
        }
